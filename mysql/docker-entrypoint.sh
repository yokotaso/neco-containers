...
#		chown -R mysql:mysql "$DATADIR"
		...
		mysql=( mysql --defaults-extra-file="$PASSFILE" --protocol=socket -uroot -hlocalhost --socket="$SOCKET" --init-command="SET @@GLOBAL.SUPER_READ_ONLY=OFF; SET @@GLOBAL.OFFLINE_MODE=OFF; SET @@SESSION.SQL_LOG_BIN=0;")
...
EOF
		...
		if [ "$MYSQL_REPLICATION_USER" -a "$MYSQL_REPLICATION_PASSWORD" ]; then
			echo "CREATE USER '"$MYSQL_REPLICATION_USER"'@'%."$MYSQL_REPLICATION_DOMAIN"' IDENTIFIED BY '"$MYSQL_REPLICATION_PASSWORD"' ;" | "${mysql[@]}"
			echo "GRANT REPLICATION SLAVE ON *.* TO '"$MYSQL_REPLICATION_USER"'@'%."$MYSQL_REPLICATION_DOMAIN"' ;" | "${mysql[@]}"
		fi
